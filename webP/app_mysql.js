...
app.get('/topic/add',function(req,res){ // add페이지로 들어갈때
    console.log("들어왔다.");
    ...
    conn.query(sql,function(err,topics,fields){ // topics안에 sql문 데이터가 담겨져 있다.
        ...
        res.render('add',{topics:topics}); // 데이터를 보낼때
});
});
...
app.get(['/topic','/topic/:id'], function(req,res){
    ...
    conn.query(sql,function(err,topics,fields){
        ...
        if(id){
            ...
            res.render('view',{topics:topics}); // view 표시
        }
...
app.post('/topic/add', function(req,res){ //add해서 다시 메인화면으로 넘어갈때
            ...
            if(err){
                ...
                if(topic.length === 0){
                    ...
                    res.status(500).send('Internal Server Error');
                          ...
                          res.render('delete', {topics:topics, topic:topic[0]})// id에 해당하는 삭제할 데이터값 다시 응답해줌 그럼 삭제버튼 누르게 되는 post방식 delete로 넘어감 
                }
...
app.get('/topic/:id/edit', function(req,res){ //edit페이지 들어왔을때
        ...
        if(id){
                ...
                conn.query(sql,[id], function(err,topic,fields){
                    if(err){
                        ...
                        res.render('edit',{topics:topics, topic:topic[0]});// 다시 클라이언트측에 응답 topics,topic데이터를 가지고
